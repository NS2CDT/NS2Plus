...
function GUIScoreboard:UpdateTeam(updateTeam)
	...
	if #playerList ~= #teamScores then
		self:ResizePlayerList(playerList, #teamScores, teamGUIItem)
	end

	local skillsum = 0
	local invalidPlayers = 0
	for i = 1, #playerList do
		local player = playerList[i]
		local record = teamScores[i]
		local client = record.ClientIndex
		...
		if self.showPlayerSkill and record.Skill >= 0 then
			player.Name:SetText(string.format("[%s] %s", record.Skill, player.Name:GetText()))
		end

		if record.Skill >= 0 then
			skillsum = skillsum + record.Skill
		else
			invalidPlayers = invalidPlayers + 1
		end
	end
	-- Assume player who have invalid hive skill have the average amount
	-- of hive skill on the team. Not super accurate, but better than nothing.
	skillsum = skillsum + skillsum / (#playerList - invalidPlayers) * invalidPlayers
	...
	if (teamNumber == 1 or teamNumber == 2) and (self.showAvgSkill or self.showProbability) then
		...
		local probability = 1 / (1 + math.exp(
			(team2Skill - team1Skill) / ((#self.teams[2].PlayerList + #self.teams[3].PlayerList) * 100)
		))
		team1VictoryP = Round(probability * 100)
	end
end
